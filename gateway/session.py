...
class SessionSource:
    ...
    is_bot: bool = False
    # Platform-neutral workspace/server discriminator.
...
def neutralize_untrusted_inline_text(
    value: Any, *, max_chars: int = _MAX_PROMPT_METADATA_CHARS
) -> str:
    """Collapse untrusted text to a single inert line."""
    text = str(value).replace("\r\n", "\n").replace("\r", "\n").replace("\n", " ")
...
def build_session_context_prompt(
    ...
    *,
    ...
    Platforms outside the PII-safe set keep real IDs when mentions require them.
...
class SessionEntry:
    ...
    # (e.g. Mattermost thread-context watermarks). Survives gateway restarts via
    ...
    # session it replaced. Used to give messaging channels/threads a
...
def build_session_key(
    ...
    Group/channel rules:
      ...
      - thread_id differentiates threads within that parent chat.  When
        ...
        conversations (Telegram topics and Mattermost threads).
...
class SessionStore:
    ...
    def _find_gateway_session_row(
        ...
        *,
        ...
        Callers can disable SessionDB's platform/chat/user fallback when only
        an exact session-key recovery is safe.
        """
        if not self._db:
            return None
