...
from gateway.platforms.telegram.telegram_network import (
    SEED_FALLBACK_IPS,
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def on_processing_complete(self, event: MessageEvent, outcome: ProcessingOutcome) -> None:
        ...
        Telegram's set_message_reaction replaces all existing reactions in one
        call, so no separate remove step is needed.
...
# ──────────────────────────────────────────────────────────────────────────
# Internal setup and configuration helpers
# ──────────────────────────────────────────────────────────────────────────
...
def is_connected(config) -> bool:
    ...
    gateway configuration would enable
...
def apply_yaml_config(yaml_cfg: dict, telegram_cfg: dict) -> dict | None:
