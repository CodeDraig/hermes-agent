...
class MattermostAdapter(BasePlatformAdapter):
    ...
    async def _handle_ws_event(self, event: Dict[str, Any]) -> None:
        ...
        team_id = str(data.get("team_id") or post.get("team_id") or "") or None
...
def interactive_setup() -> None:
    ...
    Lazy-imports CLI helpers so the adapter's import surface stays small,
    prompts for the
...
def apply_yaml_config(yaml_cfg: dict, mattermost_cfg: dict) -> dict | None:
    """Translate ``config.yaml`` ``mattermost:`` keys into env vars.

    Called directly by ``gateway.config`` while loading retained transports.
