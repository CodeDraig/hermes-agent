...
def _missing_log_note(log_name: str) -> str:
    """Explain a missing log without raising."""
...
def collect_share_bundle(
    ...
    where each value is the already-redacted (when
...
class DebugShareResult:
    ...
    Returned by :func:`build_debug_share` so non-CLI callers can consume the
    uploaded paste URLs without scraping printed text.
    """
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``hermes debug share``. It performs
    blocking network I/O (paste uploads), so callers inside an event loop must
    ...
    for label in ("agent.log", "gateway.log"):
