...
Usage:
    ...
    hermes honcho migrate                  # Step-by-step Honcho migration guide
...
def _pin_kanban_board_env() -> None:
    """Pin the active Kanban board for the lifetime of this CLI session.

    In-process tools and shelled-out ``hermes kanban`` calls otherwise reread
    the mutable global board pointer independently. Pinning at startup keeps a
    concurrent board switch from splitting one conversation across boards.
    """
...
def cmd_chat(args):
    ...
    # Maps to create_agent(skip_context_files=True, skip_memory=True).
...
def cmd_gateway(args):
    ...
    # Gateway may be a user's first Hermes entrypoint. Seed bundled skills,
    # but do not make gateway startup depend on an optional content sync.
    try:
        _sync_bundled_skills_for_startup()
    except Exception:
        pass
...
def select_provider_and_model(args=None):
    ...
    canonical_descs = {p.slug: p.picker_desc for p in CANONICAL_PROVIDERS}
...
# The main.py decomposition moved the sessions/update command
...
def _quarantine_running_hermes_exe(
    ...
    handles (recovers in <1s), or another Hermes process using the same
    install. We mitigate:
    ...
    2. If all retries fail, schedule the .exe for replacement on next
       ...
       the stale image.
    3. Print a clear warning naming the likely gateway/REPL culprit and
       pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Exit other `hermes` REPLs, stop the "
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # partial installs where a newly added base dependency
    ...
    # hours later inside ``hermes update`` or skill-sync
...
def _resolve_node_runtime_npm() -> str | None:
    ...
    symlink and path errors in source trees. Refuse a
...
def cmd_update(args):
    ...
    # Cross-process mutual exclusion: two terminal updates mutating one
    # checkout can rewrite source under a live interpreter and strand the tree
    # half-updated.
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        ...
        "resume",
        "send", "sessions", "setup",
        "skin", "skills", "status", "tools", "uninstall", "update",
...
def _prepare_agent_startup(args) -> None:
    ...
    try:
        ...
        # Discovery runs in a daemon thread so its manifest scanning and plugin
        # imports overlap the rest of prompt-toolkit startup.
...
def main():
    ...
    # =========================================================================
    # pets command — petdex animated mascots for the interactive CLI
    # =========================================================================
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Hermes shows it reacting to agent activity in the CLI."
        ),
    )
    try:
