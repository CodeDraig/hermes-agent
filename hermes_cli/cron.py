...
def _active_cron_provider_name() -> str:
    """Name of the resolved cron scheduler provider.
...
def _warn_if_gateway_not_running() -> None:
    ...
    An external provider may fire jobs outside the in-process ticker, so a
    momentarily-absent gateway process does not
...
def cron_status():
    ...
    if provider != "builtin":
        # An external provider does NOT run the in-process 60s ticker, so there
        # is intentionally NO
        ...
        # healthy external provider. Report the provider instead and skip the
