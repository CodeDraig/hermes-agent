...
def resolve_entry_api_key(entry: dict[str, Any] | None) -> str | None:
    ...
    ``key_env`` names the environment variable holding the key. Returns None when
...
def get_fallback_chain(config: dict[str, Any] | None) -> list[dict[str, Any]]:
    """Return the configured ordered fallback provider chain."""

    config = config or {}
    return _iter_fallback_entries(config.get("fallback_providers"))
