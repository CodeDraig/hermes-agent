...
#   4. The gateway path drops files into
...
def run_uninstall(args):
    ...
    # an unattended run, so it stays opt-in to the interactive flow.
...
def _perform_uninstall(
    *,
    ...
    ``hermes`` wrapper + node symlinks → delete the code checkout →
    (Windows) remove PortableGit/Node → optionally
