...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader, including a launch-service-managed gateway that left a fresh
    ``gateway_state.json`` but no live PID file. In those cases fall back to
    validating the PID recorded
    in the profile's own ``gateway_state.json`` against the live process table,
    using the shared gateway liveness logic. Parameterized by ``profile_dir``
    so it never mutates ``HERMES_HOME``.
    """
    try:
...
def create_profile(
    ...
    # day one. Without it, profile-scoped env writes such as
    # `hermes -p <name> auth add` had no file until first
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    Profiles created before the CLI started seeding a ``.env`` (PR #44792)
    have none. Once credential writes became profile-scoped those profiles
    stopped inheriting the root install's credentials. To avoid breaking
...
def _rmtree_with_retry(profile_dir: Path, onexc_handler) -> None:
    ...
    Even after stopping the gateway, a just-terminated
