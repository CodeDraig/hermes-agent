...
_PLATFORMS = [
    ...
    {"key": "mattermost", "label": "Mattermost", "emoji": "💬"},
]


def _all_platforms() -> list[dict]:
    """Return the retained human messaging platforms for setup menus."""
    return [dict(platform) for platform in _PLATFORMS]


def _platform_status(platform: dict) -> str:
    """Return whether a retained platform has usable credentials."""
    key = platform["key"]
    try:
        from gateway.config import load_gateway_config, Platform
        config = load_gateway_config().platforms.get(Platform(key))
        if key == "telegram":
            from gateway.platforms.telegram.adapter import is_connected
        else:
            from gateway.platforms.mattermost import is_connected
        return "configured" if config and is_connected(config) else "not configured"
    except Exception:
...
def _configure_platform(platform: dict) -> None:
    """Run the retained platform's internal setup flow."""
    if platform["key"] == "telegram":
        from gateway.platforms.telegram.adapter import interactive_setup
    elif platform["key"] == "mattermost":
...
def gateway_setup():
    ...
    # Consider either retained human messaging platform configured once the
    # user has made meaningful setup progress.
