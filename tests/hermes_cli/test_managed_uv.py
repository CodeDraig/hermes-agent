...
class TestMinorLineFallForward:
    ...
    def test_returns_none_with_bounded_attempts_when_all_minors_exhausted(
        ...
        """When every build on every supported minor line (3.11-3.15) is
        ...
        # patches; five lines total (3.11 through 3.15) and nothing beyond
        # 3.15 (requires-python is <3.16).
        assert install_calls.count("3.11") == 1
        assert install_calls.count("3.12") == 1
        assert install_calls.count("3.13") == 1
        assert install_calls.count("3.14") == 1
        assert install_calls.count("3.15") == 1
        assert not any(call.startswith("3.16") for call in install_calls)
        for minor in (11, 12, 13, 14, 15):
            explicit = [
        ...
        assert len(install_calls) <= 5 * (1 + cap)
