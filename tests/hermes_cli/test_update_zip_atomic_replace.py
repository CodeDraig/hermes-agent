...
which could leave any replaced package directory missing.
...
def test_atomic_replace_swaps_content_on_success(tmp_path: Path) -> None:
    ...
    assert not (dst.parent / "bundle.hermes-update-staging").exists()
    assert not (dst.parent / "bundle.hermes-update-old").exists()
