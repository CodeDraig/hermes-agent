...
from agent import init_context


def test_warns_once_and_dedupes(caplog):
    init_context._warned_unavailable_providers.clear()
    with caplog.at_level(logging.WARNING, logger="run_agent"):
        init_context._warn_memory_provider_unavailable("hindsight")
        init_context._warn_memory_provider_unavailable("hindsight")
...
def test_distinct_providers_each_warn(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="run_agent"):
        init_context._warn_memory_provider_unavailable("hindsight")
        init_context._warn_memory_provider_unavailable("mem0")
...
def test_provider_reason_is_appended(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="run_agent"):
        init_context._warn_memory_provider_unavailable("hindsight", hint)
